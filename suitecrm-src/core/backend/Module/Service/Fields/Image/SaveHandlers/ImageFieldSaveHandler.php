...
namespace App\Module\Service\Fields\Image\SaveHandlers;
...
class ImageFieldSaveHandler implements RecordFieldTypeSaveHandlerInterface
{
    ...
    public function run(?Record $previousVersion, Record $inputRecord, ?Record $savedRecord, FieldDefinition $fieldDefinition, string $field): void
    {
        ...
        if (empty($mediaObjectId)) {
            ...
            foreach ($legacyMediaObjects as $legacyMediaObject) {
                // This will delete the media object from the repository and file system
                $this->mediaObjectManager->deleteMediaObject('legacy-images', $legacyMediaObject);
            }
