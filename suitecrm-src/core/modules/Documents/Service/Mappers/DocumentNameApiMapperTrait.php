...
trait DocumentNameApiMapperTrait
{
    /**
     * @param Record $revisionRecord
     * @param Record $documentRecord
     * @param string $revisionField
    ...
    protected function injectMediaObject(
        ...
        $module = $moduleNameMapper->toLegacy($revisionRecord->getModule());
        ...
        $mediaObjectRecord = $mediaObjectManager->mapToRecord($storageType, $mediaObjects[0]);

        if (!$mediaObjectRecord) {
            return;
        }

        $documentAttributes = $documentRecord->getAttributes();
        $documentAttributes[$documentField] = $mediaObjectRecord->toArray();

        $documentRecord->setAttributes($documentAttributes);
    }
}
