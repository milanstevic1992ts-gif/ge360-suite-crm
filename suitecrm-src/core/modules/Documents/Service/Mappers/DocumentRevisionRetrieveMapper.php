...
class DocumentRevisionRetrieveMapper implements ApiRecordFieldMapperInterface {
    ...
    {
        return 'documents-revision-retrieve-mapper';
    }
    ...
    public function toExternal(Record $record, FieldDefinition $fieldDefinitions): void
    {
        ...
        $revision = $this->documentsManager->getLatestRevision($documentId);

        $documentAttributes = $record->getAttributes();
        $documentAttributes[$this->getField()] = $revision;

        $record->setAttributes($documentAttributes);
    }
