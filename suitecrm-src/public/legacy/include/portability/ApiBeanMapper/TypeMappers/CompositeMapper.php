...
class CompositeMapper implements TypeMapperInterface
{
    /**
     * @inheritDoc
     */
    ...
    {
        $fieldName = !empty($alternativeName) ? $alternativeName : $name;
        $value = $bean->$name ?? '';
    ...
    public function toBean(SugarBean $bean, array &$container, string $name, string $alternativeName = ''): void
    {
        ...
        if (is_array($value)) {
            $container[$fieldName] = json_encode($value);
            return;
        }

        // Already a string (or empty) — leave as-is
    }
}
